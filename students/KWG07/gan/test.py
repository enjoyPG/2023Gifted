...
netG.load_state_dict(torch.load('gan/weights/netG_epoch_99.pth'))
print(netG)
...
netD.load_state_dict(torch.load('gan/weights/netD_epoch_99.pth'))
print(netD)
...
for epoch in range(niter):
    for i, data in enumerate(dataloader, 0):
        ...
        if i % 100 == 0:
            vutils.save_image(real_cpu,'gan/output/real_samples.png' ,normalize=True)
            fake = netG(fixed_noise)
            vutils.save_image(fake.detach(),'gan/output/fake_samples_epoch_%03d.png' % (epoch), normalize=True)        
    torch.save(netG.state_dict(), 'gan/weights/netG_epoch_%d.pth' % (epoch))
    torch.save(netD.state_dict(), 'gan/weights/netD_epoch_%d.pth' % (epoch))
